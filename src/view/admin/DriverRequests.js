...
function DriverRequests() {
  ...
  const getAllDrivers = async () => {
    await axios
      .get(process.env.REACT_APP_API_URL + "/drivers/signup-requests")
      .then((res) => {
  ...
  const handleFilter = async (e) => {
    ...
    await axios
      .get(
        `${process.env.REACT_APP_API_URL}/drivers/signup-requests?level=${level}`
      )
      .then((res) => {
        setDriverList(res.data);
        setDrivers(res.data.slice(0, 10));
    ...
    return (
      <ul className="pagination justify-content-center">
        ...
        <li className="page-item">
          <button
            className="page-link"
            onClick={() => handlePageRedirect(curPage - 1)}
          >
            <AiOutlineLeft />
          </button>
        </li>
        ...
        <li className="page-item">
          <button
            className="page-link"
            onClick={() => handlePageRedirect(totalPage)}
          >
            <AiOutlineDoubleRight />
          </button>
        </li>
    ...
    <AdminLayout page_title="TaJa_ドライバー_承認">
      ...
      <div className="d-flex ms-5">
        <div className="ms-5">
          合計:
          <span
            className="btn border ms-2 rounded-0"
      ...
      <div className="ms-5" style={{ marginTop: "11px" }}>
        <table
          className="table table-borderless border border-dark text-center ms-5"
          style={{ maxWidth: "80%", fontSize: "15px" }}
        >
          <thead>
            <tr className="border-bottom border-dark">
              <th
                className="border-end border-dark table-header"
                style={{ width: "15%" }}
              >
                序数
              </th>
